...
setwd("~/github/metric-proficiency--scientific-literacy-and-attitude/2017")
...
tb.ori = read.csv("metric_responses20171019.csv", colClass=rep("character", 24))
#str
#str(tb.ori); 
#tb.ori$Timestamp
...
for( i in 5:length(tb[, 1])) {  #outter for-loop 
  for( j in 5:length(tb[1, ])) {  #inner for-loop
    if (debug > 0) { print( paste("i=", i, "j=", j) ) }
  }
}
